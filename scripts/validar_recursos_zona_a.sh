...
security_group_id=$(aws ec2 describe-security-groups --group-names "approyal" --query "SecurityGroups[0].GroupId" --output text 2>/dev/null)
if [ $? -eq 0 ]; then
  echo "[OK] Security Group approyal foi criado"
  
  # Validar inbound rule para o security group 'approyal'
  ...
  # Validar outbound rule para o security group 'approyal'
  ...
  echo ">[ERRO] Não achei o security group approyal. Ele foi criado?"
fi
